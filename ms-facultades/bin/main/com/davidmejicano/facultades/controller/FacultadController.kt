...
class FacultadController(private val repository: FacultadRepository, private val carreraRepository: CarreraRepository) {
    ...
    @Put("/{id}")
    fun actualizar(@PathVariable id: Long, @Body actualizado: Facultad): HttpResponse<Facultad> {
        ...
        val modificado = existente.copy(
            ...
            siglas = actualizado.siglas ?: existente.siglas,
            nombreDecano = actualizado.nombreDecano ?: existente.nombreDecano,
            telefono = actualizado.telefono ?: existente.telefono,
            correo = actualizado.correo ?: existente.correo
        )

        return HttpResponse.ok(repository.update(modificado))
    }
